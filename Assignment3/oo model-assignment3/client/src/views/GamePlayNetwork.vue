  ...
  <div class="game-play-network">
    ...
    <div v-else-if="gameStore.status === 'WAITING'" class="waiting-container">
      ...
      <div class="players-waiting">
        <div v-for="player in gameStore.players" :key="player.id" class="player-waiting">
          ✓ {{ player.name }}
        </div>
        <div 
      ...
      <button 
        ...
        class="start-button"
      >
        {{ gameStore.players.length < 2 ? 'Need at least 2 players' : 'Start Game' }}
      </button>
    ...
    <div v-else-if="gameStore.status === 'IN_PROGRESS'" class="game-container">
      ...
      <div class="game-board">
        <div class="discard-pile">
          ...
          <div v-if="gameStore.currentColor" class="current-color" :style="{ backgroundColor: getColorHex(gameStore.currentColor) }">
    ...
    <!-- Game Finished -->
    <div v-else-if="gameStore.status === 'FINISHED'" class="game-over-container">
      <h1>🎉 Game Over! 🎉</h1>
...
const canCallUno = computed(() => 
  gameStore.playerHand.length === 2 && gameStore.isMyTurn
)
...
function isCardPlayable(card) {
  ...
  // Match value/type
  if (card.value === top.value || card.type === top.type) {
    return true
  }
